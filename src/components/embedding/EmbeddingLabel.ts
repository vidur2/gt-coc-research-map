...
import type { Rect } from '../../types/common-types';
import type {
  DrawnLabel,
  LabelData,
  TopicData
...
import { allTrue, anyTrue, rectsIntersect } from '../../utils/utils';
...
export function layoutTopicLabels(
  ...
  canSkipLayout = true,
  useAllGridTopics = true
) {
  if (this.topicLevelTrees.size <= 1) return;
  if (!this.showLabel) return;

  // Calculate relative zoom level and hide labels if between 0.5x and 1.5x
  const relativeZoom = this.curZoomTransform.k / this.initZoomTransform.k;
  if (relativeZoom >= 0.5 && relativeZoom <= 1.5) {
    // Hide all labels by clearing the topic group
    const topicGroup = this.topSvg.select('g.top-content g.topics');
    topicGroup.selectAll('g.label-group').remove();
    return;
  }

  // Only check contours if we're using the old high-density mode
  if (!useAllGridTopics) {
  ...
  // Collect polygon centers from the total contour OR all grid topics
  let polygonCenters: [number, number, number][] = [];

  if (useAllGridTopics) {
    // Use all grid topics instead of just high-density regions
    const idealTreeLevel = this.getIdealTopicTreeLevel()!;
    const topicTree = this.topicLevelTrees.get(idealTreeLevel)!;
    const curZoomBox = this.getCurZoomBox();

    // Get all topics in the current view
    const allTopics = topicTree.data().filter(d => {
      const x = this.xScale(d[0]);
      const y = this.yScale(d[1]);
      return x >= curZoomBox.x && x <= curZoomBox.x + curZoomBox.width &&
        y >= curZoomBox.y && y <= curZoomBox.y + curZoomBox.height;
    });



    // Convert to the expected format with equal weights
    polygonCenters = allTopics.map(d => [this.xScale(d[0]), this.yScale(d[1]), 1.0]);
  } else {
    // Original behavior: use contour polygon centers
