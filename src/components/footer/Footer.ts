...
export class Footer {
  ...
  promptPoints: PromptPoint[] | undefined;
  footerUpdated: () => void;
  ...
  initStore = () => {
    this.footerStore.subscribe(value => {
      ...
      // Calculate relative zoom (current zoom / initial zoom)
      const relativeZoom = this.footerStoreValue.curZoomTransform.k / this.footerStoreValue.initZoomK;
      this.zoomLevel = `${relativeZoom.toFixed(1)}x`;

      this.embeddingName = this.footerStoreValue.embeddingName;
      this.promptPoints = this.footerStoreValue.promptPoints;
